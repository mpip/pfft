...
program main
  ...
  alloc_local = pfft_local_size_dft_r2c(4, n, comm_cart_3d, PFFT_TRANSPOSED_OUT, &
      local_ni, local_i_start, local_no, local_o_start)

  ! Allocate enough memory for both trafos
  cin  = pfft_alloc_real(2 * alloc_local)
  ...
  plan_forw = pfft_plan_dft_r2c( &
      4, n, in, out, comm_cart_3d, PFFT_FORWARD, PFFT_TRANSPOSED_OUT + PFFT_MEASURE + PFFT_DESTROY_INPUT)

  ! Plan parallel backward FFT
  plan_back = pfft_plan_dft_c2r( &
      4, n, out, in, comm_cart_3d, PFFT_BACKWARD, PFFT_TRANSPOSED_IN + PFFT_MEASURE + PFFT_DESTROY_INPUT)

  ! Initialize input with random numbers
  call pfft_init_input_r2c(4, n, local_ni, local_i_start, &
      in)
