...
int main(int argc, char **argv)
{
  ...
  double err;
  pfft_complex *planned_in, *executed_in;
  ...
  /* Allocate memory */
  planned_in  = pfft_alloc_complex(alloc_local);
  planned_out = pfft_alloc_complex(alloc_local);

  /* Plan parallel forward FFT */
  plan_forw = pfft_plan_dft_3d(
      n, planned_in, planned_out, comm_cart_2d, PFFT_FORWARD, PFFT_TRANSPOSED_OUT| PFFT_MEASURE| PFFT_DESTROY_INPUT);
  
  /* Plan parallel backward FFT */
  plan_back = pfft_plan_dft_3d(
      n, planned_out, planned_in, comm_cart_2d, PFFT_BACKWARD, PFFT_TRANSPOSED_IN| PFFT_MEASURE| PFFT_DESTROY_INPUT);
  ...
  executed_in  = pfft_alloc_complex(alloc_local);
  ...
  pfft_execute_dft(plan_forw, executed_in, executed_out);

  /* execute parallel backward FFT */
  pfft_execute_dft(plan_back, executed_out, executed_in);
  ...
  err = pfft_check_output_complex_3d(n, local_ni, local_i_start, executed_in, comm_cart_2d);
