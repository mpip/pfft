...
#include <pfft.h>
\end{lstlisting}
in preamble of each file calling PFFT. This header includes \code{fftw3-mpi.h} and \code{fftw.h} automatically.
Therefore, PFFT uses the \code{fftw_complex} data type defined in \code{fftw.h}. For the sake of a clean namespace we
define the data type \code{pfft_complex} via
\begin{lstlisting}
typedef fftw_complex pfft_complex;
\end{lstlisting}
as a simple wrapper of \code{fftw_complex}.
...
\subsection{Data Distribution}\label{sec:gc:local_size}
\begin{lstlisting}
