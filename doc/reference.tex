...
\begin{compactitem}
  ...
  \item \code{PFFT_PADDED_R2C}, \code{PFFT_PADDED_C2R} - real array uses padding similar to FFTW-MPI
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \code{howmany} is the number of transforms to compute. The resulting plan computes howmany transforms, 
        ...
        The basic \code{pfft_plan_dft_r2c} interface corresponds to howmany=1.
  ...
  \item \code{in} and \code{out} point to the input and output arrays of the transform, which may be the same (yielding an in-place transform).
...
\todo[inline]{default - real array does not use padding}
\todo[inline]{\code{PFFT_PADDED_R2C}, \code{PFFT_PADDED_C2R} - real array uses padding similar to FFTW-MPI}
