...
\subsection{Generating periodic Cartesian communicators}
\begin{lstlisting}
int pfft_create_procmesh_2d(
    MPI_Comm comm, int np0, int np1,
    MPI_Comm *comm_cart_2d);
...
\subsection{Determine the parallel data decomposition}
...
the local offset $local_gc_start[rnk_n]$. Every process gets the data 
...
every process owns the data \code{data[k[0],k[1],k[2]]} with \code{local_gc_start[t] <= k[t] < local_gc_start[t] + local_ngc[t]}
for \code{t=0,...,rnk-1}. Hereby, the index k wraps periodically if \code{n[t] < local_gc_start[t] + local_ngc[t]}.
...
call of a \code{pff_local_size} function, see \ref{sec:local_size}.
