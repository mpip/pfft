...
[
  ...
  if test "$with_vtk" != "no"; then
    ...
    vtk_save_CPPFLAGS=$CPPFLAGS
    CPPFLAGS="$CPPFLAGS -I$VTK_PREFIX/include/vtk$vtk_suffix"
    AC_CHECK_HEADER([vtkCommonInstantiator.h], [vtkFound="OK"])
    CPPFLAGS=$vtk_save_CPPFLAGS
    ...
    if test -z "$vtkFound"; then
      ...
      VTK_LIBS="-lvtkDICOMParser -lvtkFiltering -lvtkfreetype -lvtkftgl -lvtkGraphics -lvtkHybrid -lvtkImaging -lvtkIO -lvtkjpeg -lvtkpng -lvtkRendering -lvtktiff -lvtkzlib -lvtkCommon -lvtksys"

      dnl set VTK c,cpp,ld flags
      VTK_CFLAGS="-I$VTK_PREFIX/include/vtk$vtk_suffix"
      VTK_CXXFLAGS="$VTK_CFLAGS"
      VTK_LDFLAGS="-L$VTK_PREFIX/lib/vtk$vtk_suffix"
      ...
      if test -n "$1"; then
        ...
        AC_COMPILE_IFELSE([AC_LANG_PROGRAM(
              ...
              [
                ...
                #error Installed VTK is too old !
		#endif
                #if VTK_MAJOR_VERSION == $maj && VTK_MINOR_VERSION == $min && VTK_BUILD_VERSION < $rel
                #error Installed VTK is too old !
                #endif
              ])
        ], [vtkVersion="OK"])
